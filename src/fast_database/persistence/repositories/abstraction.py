...
``fast_database.persistence.repositories.filter_operator.FilterOperator``.
...
class IRepository(ContextMixin):
    ...
    Note:
        ...
        ``fast_database.persistence.repositories.filter_operator.FilterOperator`` for operator
    ...
    def retrieve_record_by_filter(
        ...
        if not include_deleted:
            query = self._filter_active_non_deleted(query)
    ...
    def delete_record_by_filter(
        ...
        if hard_delete:
            ...
            elif hasattr(record, "updated_on"):
